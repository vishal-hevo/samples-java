...
public class IncrementalWorkflowImpl implements IncrementalWorkflow {
  private final Logger logger = Workflow.getLogger(IncrementalWorkflowImpl.class);
  ...
  public void execute(String integrationID) {
    ...
    logger.info("started workflowID: {}, integrationID {} ", workFlowID, integrationID);

    var mutexWorkflowExecution = tryAcquireLock(integrationID, workflowTimeoutDuration);
    boolean isLockAcquired = isLockAcquiredByWorkflow(mutexWorkflowExecution);

    if (!isLockAcquired) {
      logger.info("skipping execution for workflow {}", workFlowID);
      return;
    }

    logger.info("critical operation started");
    Workflow.sleep(Duration.ofSeconds(20L));
  ...
  private WorkflowExecution tryAcquireLock(String resourceID, Duration lockDuration) {
    return activities.signalWithStartMutexWorkflow(resourceID, lockDuration, false);
  }

  private boolean isLockAcquiredByWorkflow(WorkflowExecution mutexWorkflowExecution) {
    return activities.isLockAcquired(mutexWorkflowExecution);
  }
